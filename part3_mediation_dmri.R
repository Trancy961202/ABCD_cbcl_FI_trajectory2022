load("/home/luoqiang/lqy/r0901/step3/dmri/mediation/r0904_cbclfdr_FIfdr_dmri_cor_sig.RData")
load("/home/luoqiang/lqy/r0901/step1/demo/r0902_dmri.RData")
...
z.name <- rep()
n <- 0
for (i in 1:nrow(cbclfdr_FIfdr_dmri_cor_sig)){
  n <- n + 1
  x.name[n] <- cbclfdr_FIfdr_dmri_cor_sig[i,]$dmri
  y.name[n] <- cbclfdr_FIfdr_dmri_cor_sig[i,]$cbcl
  z.name[n] <- cbclfdr_FIfdr_dmri_cor_sig[i,]$FI
}
outfile.name <- 'mediation_dmri0904.csv'
assList.cbcl.sex.dmri<- data.frame(x.name, y.name, z.name)
results_analysis_cog <- multiAss.parallel(assList.cbcl.sex.dmri, df, outfile.name)
stopImplicitCluster()
save.image("result0904_mediation_dmri.RData")