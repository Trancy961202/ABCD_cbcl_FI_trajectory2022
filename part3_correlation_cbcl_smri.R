...
###  3.1 association between cbcl and smri #########

x.names <- colnames(data_com_smri_tbss)[c(num_smri_total,num_thick,num_area,num_vol)]
y.names <- colnames(data_com_smri_tbss)[num_cbcl]
#paste(paste("site",0,2:9, sep =""),collapse = "+")
#paste(paste("site",10:22, sep =""),collapse = "+")
...
outfile.name <- 'cbcls_smri_correlation0901.csv'
...
stopImplicitCluster()
save.image("result0901_cbcls_smri_correlation.RData")